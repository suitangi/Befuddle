...
const befuddleAppVersion = "2025.7.4";
...
const discordSdkInstance = new discordSdk();

async function init() {
    try {
        await discordSdkInstance.ready();
        const user = await discordSdkInstance.commands.getUser();
        console.log("Player in Discord:", user);
    } catch (err) {
        console.log("Not running inside Discord Activity:", err.message);
    }
}

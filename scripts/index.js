...
const befuddleAppVersion = "2026.03.17";
...
function menuModal() {
  ...
  let menuD = $.dialog({
    ...
    content: '<div class="modalTitle" style="text-align: center;font-size: 30px;">Befuddle</div>' +
      ...
      '<div class="hr"></div>' +
      '<div class="modalText" id="legal">Legal <span id="legalExpand" class="material-symbols-outlined"> expand_more </span></div>' +
      '<div id="legalText" class="expandiv collapsediv">' +
      '•<a href="../privacy-policy/" target="_blank"><span>Privacy Policy</span></a>' +
      '<br>•<a href="../ToS/" target="_blank"><span>Terms of Service</span></a><br><br></div>' +
      '<div class="hr"></div>' +
      ...
      '<br><a href="https://discord.com/oauth2/authorize?client_id=1389389854747525152&permissions=18432&integration_type=0&scope=applications.commands+bot" target="_blank"><span>Discord Integration</span></a>' +
    ...
    onContentReady: function () {
      ...
      document.getElementById('legal').addEventListener('click', function () {
        let s = document.getElementById('legalText');
        if (!s.classList.contains('collapsediv')) {
          s.classList.add('collapsediv');
          document.getElementById('legalExpand').classList.remove('rotato');
        } else {
          s.classList.remove('collapsediv');
          document.getElementById('legalExpand').classList.add('rotato');
        }
      });
...
$(document).ready(async function () {
  ...
  // Set version number in loading screen
  $('#app-version').text(befuddleAppVersion);

  // Update loading subtitle based on platform
  if (!window.isDiscord) {
    document.getElementById('loading-subtitle').innerHTML = 'Befuddle is now available as an <a href="https://discord.com/oauth2/authorize?client_id=1389389854747525152">Activity on Discord</a>!';
  }
  ...
  //Get rid of loading screen regardless
  setTimeout(() => {
    toggleLoadingScreen(false);
  }, 8000);
